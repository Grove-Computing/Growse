...
起動すると、戻る・進む・再読込・URL入力欄・Gopher ボタンを備えたブラウザウィンドウが表示されます。URLを入力してGopherボタンを押すとHTMLを取得し、`x/net/html` の解析結果をGrowse独自DOMへ変換します。現在はページタイトル・DOMノード数・要素数・HTTP情報をViewportへ表示します。DOMのレイアウトと描画は次の実装段階です。

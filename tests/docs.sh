...
require() {
    ...
    if ! grep -Fq -- "$value" "$file"; then
        echo "${file}にv0.9.0文書の必須記述がありません: ${value}" >&2
        exit 1
    fi
}
...
require README.md "GROWSE_VERSION=v0.9.0"
require README.md "growse:v0.9.0"
...
require docs/form-fetch-cookie-support.md "Growse v0.9.0"
...
require docs/storage-cache-support.md "Growse v0.9.0"
...
for file in README.md SECURITY.md docs/form-fetch-cookie-support.md; do
    if grep -Eq 'GROWSE_VERSION=v0\.8\.0|growse:v0\.8\.0|Growse v0\.8\.0のYaegi|Growse v0\.8\.0の実装を基準' "$file"; then
        echo "${file}にv0.8.0向けの現行記述が残っています" >&2
        exit 1
    fi
done

echo "v0.9.0文書同期検証成功: README, SECURITY.md, Storage / Cache対応表, Form / Fetch / Cookie対応表, CSS対応表, WPT文書"

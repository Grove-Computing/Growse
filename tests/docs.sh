...
require() {
    ...
    if ! grep -Fq -- "$value" "$file"; then
        echo "${file}にv0.8.0文書の必須記述がありません: ${value}" >&2
        exit 1
    fi
}
...
require docs/form-fetch-cookie-support.md "Growse v0.8.0"
...
echo "v0.8.0文書同期検証成功: README, SECURITY.md, Form / Fetch / Cookie対応表, CSS対応表, WPT文書"

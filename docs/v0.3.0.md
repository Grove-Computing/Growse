...
このリリースには、[Issue #5「マウスカーソルがgopher君」](https://github.com/Grove-Computing/Growse/issues/5)を含める。
...
- [x] [Issue #5](https://github.com/Grove-Computing/Growse/issues/5)を実装し、マウスカーソルが青いGopherになる
...
- [x] ポインター移動から最前面のDOM要素を特定できる
- [x] スクロール後も正しいDocument座標でHit Testingできる
- [x] hover中の要素と祖先要素を経路として保持できる
- [x] `tag:hover`を解釈できる
- [x] `#id:hover`を解釈できる
- [x] `.class:hover`を解釈できる
- [x] `tag.class:hover`を解釈できる
- [x] `:hover`の詳細度とCascadeを正しく計算できる
- [x] hover開始時にComputed Style、Layout、Display Listを更新できる
- [x] hover終了時に通常の表示へ戻せる
- [x] 同じhover経路内の移動では不要なページ再計算を行わない
- [x] hover用の一時状態がDOMのclass属性へ混入しない

### WebGo EventとBrowser UX

- [x] WebGoへ`mouseenter`イベントを配信できる
- [x] WebGoへ`mouseleave`イベントを配信できる
- [x] 祖先を含むhover経路の差分に従ってイベントを規定順で配信できる
- [x] hover handlerのpanicでGrowseが終了しない
- [x] hover中の動的要素を削除しても古いイベントを呼び出さない
- [x] リンクへのhover中に解決済み遷移先URLを表示できる
- [x] リンクから外れたら通常の状態表示へ戻せる
- [x] hoverだけでHTTPリクエストを発生させない

### 品質とリリース

- [x] Counter Demoが引き続き動作する
- [x] Todo Demoの入力、追加、完了、削除が引き続き動作する
- [x] Counter DemoとTodo Demoで`:hover`の視覚変化を確認できる
- [x] `go test ./...`が成功する
- [x] `make ci`が成功する
- [x] CIの全必須チェックが成功する
- [x] Linux、macOS、WindowsでGopherカーソルを確認できる
- [x] Linux、macOS、Windows向け成果物を生成できる
- [x] Dockerイメージを生成できる
- [x] インストーラーでv0.3.0を導入できる
- [x] READMEとSECURITY.mdがv0.3.0の仕様と一致する

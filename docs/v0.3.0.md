...
- [ ] [Issue #5](https://github.com/Growse-Project/Growse/issues/5)を実装し、マウスカーソルが青いGopherになる
- [ ] カーソル画像に`internal/ui/assets/blue.svg`を使用する
- [ ] SVGをビルドへ埋め込み、実行時に外部ファイルを要求しない
- [ ] Gopherカーソルを32dp基準かつ正しいアスペクト比で表示できる
- [ ] HiDPI環境でもカーソルの見かけ上のサイズとホットスポットが維持される
- [ ] ToolbarとViewportの双方でGopherカーソルを表示できる
- [ ] Gopher表示中はネイティブカーソルと二重表示にならない
- [ ] ウィンドウ外または初期化失敗時に安全なカーソル状態へ戻れる
- [ ] Cursor Overlayがクリック、inputフォーカス、およびスクロールを妨げない

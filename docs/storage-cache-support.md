...
この表はGrowse v0.9.0の実装を基準とする。「部分対応」は主要なPage利用経路を扱えるが、Web標準の全機能を実装していない項目を表す。

## Web Storage

WebGoは`growse/storage`の`Local()`と`Session()`からStorageを取得し、`Get`、`Set`、`Remove`、`Clear`、`Length`、`Key`を使用する。HTTP(S)のscheme、host、正規化したportからOriginを作り、異なるOriginの値を共有しない。

| 機能 | 対応 | 補足 |
| --- | --- | --- |
| Local Storage | 対応 | OSのUser Config Directory配下へOrigin単位で永続化し、Browser再起動後に復元する |
| Session Storage | 対応 | Browser Window内でOrigin単位に保持し、reloadとsame-document traversalでは維持する |
...
| 対象外 | 非対応 | Storage Event、複数Window間同期、third-party partition、利用者向けStorage管理UI |
...
Navigation、Stylesheet、Image、WebGo Source、Fetchは同じprivate HTTP Cache Policyを使用する。通常Navigationはfresh entryを再利用し、通常reloadはDocumentを再検証しつつfresh subresourceを再利用する。強制reloadはDocumentとsubresourceの両方へ再検証を要求する。

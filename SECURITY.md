...
外部Go sourceを信頼しないでください。外部JavaScriptはv0.19.0 sandbox boundary内で扱いますが、未知の実装脆弱性を想定し、機密情報を持つ高権限環境では実行しないでください。

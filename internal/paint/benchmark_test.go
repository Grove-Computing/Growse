...
// from an already resolved dashboard Layout Tree.
func BenchmarkDashboardPaint(b *testing.B) {
	tree := benchmarkDashboardTree(b, 48)
	b.ReportAllocs()
	b.ResetTimer()
	for b.Loop() {
		_ = Build(tree)
...
func benchmarkDashboardTree(tb testing.TB, count int) *layout.Tree {
	tb.Helper()
...
`))
	...
	return layout.BuildWithViewport(document, style.Compute(document, stylesheet), 1280, 720)
}

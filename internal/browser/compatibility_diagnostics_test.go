...
func TestCompatibilityDiagnosticsExposeFontChainImageCacheAndFrameReasons(t *testing.T) {
	...
	page.renderMetrics = RenderMetrics{
		...
		ImageResourceCoalesced: 4,
		ImageResourceCanceled:  3,
		ImageResourceRejected:  2,
		ImagePaintHits:         7,
		InitialRebuilds:        1,
		StyleRebuilds:          3,
		DisplayListReuses:      9,
	...
	for _, expected := range []struct {
		...
		{"resource-queue", "image", "coalesced", "in-flight", 4},
		{"resource-queue", "image", "canceled", "lifecycle", 3},
		{"resource-queue", "image", "rejected", "bounded", 2},
